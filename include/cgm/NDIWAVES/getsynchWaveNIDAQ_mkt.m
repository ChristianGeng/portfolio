function getsynchWaveNIDAQ_mkt(daqFile,smpte_chn_NI,NDIsessionroot,smpte_chn_WAVE,session,filelist,sessfile)
...
smptedescr=str2mat('hh','mm','ss','ff','audio_time');
...
cutname=[daqFile 'syncCh',num2str(smpte_chn_NI)];
...
for fnum=filelist
    ...
    %% ANALYSIS OF NATIONAL INSTRUMENTS NIDAQ SMPTE starts here:
    
    NIDAQ=load([daqFile int2str0vec(filelist(fnum))]);
    private.NIDAQ=NIDAQ.private;
    ...
    %% NATIONAL INSTRUMENTS NIDAQ Ends here:
    %%
    totalcuts=totalcuts+1;
    
    
    
    %% ANALYSIS of NDI WAVE SMPTE starts here
    ...
    %% ANALYSIS of NDI WAVE SMPTE ENDS here
    ...
    disp(smptedescr);    
    disp('NI first and last frame:')
    disp(smpte_NI([1 end],:));
    
    disp('WAVE first and last frame:')
    disp(smpte([1 end],:));

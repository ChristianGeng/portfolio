...
%notifytime=srate/4;
%set(NIsession,'NotifyWhenDataAvailableExceeds',notifytime);
 %2560
devs=daq.getDevices;

% first  nchanNI9234 channels
%[channels,indexes]=NIsession.addAnalogInputChannel(devs(1).ID,0:nchanNI9234-1,'voltage');
% second channel
[channels,indexes]=NIsession.addAnalogInputChannel(devs(1).ID,1,'voltage');
...
set(channels(1),'Name',str2mat('Audio Participant'))

%NIsession.removeChannel(1)
set(NIsession,'Rate',srate)
[lh]=NIsession.addlistener('DataAvailable', @dataaqNIonly);

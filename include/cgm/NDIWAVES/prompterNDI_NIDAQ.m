function prompterNDI_NIDAQ(stimfile,logfile,trial_duration,fontsize,aDaqNameCommon,startwav,endwav);
% prompterNDI_NIDAQ Prompt program for NDI wave system, also controlling
...
if nargin>5
    ...
    if ~isempty(wavstart)
        ...
        playerStart = audioplayer(wavstart_d,wavstart_sf);
    end;
end;
...
if nargin>6
    ...
    if ~isempty(wavend)
        ...
        playerEnd = audioplayer(wavend_d,wavend_sf);
    end;
end;

% prepare COMPACTDAQ 
daqNameCommon='daqdat_';
if nargin>4
 daqNameCommon=aDaqNameCommon;
end


%% SETUP DAQ
disp('DAQ initialization ...')
...
daqdatindex=0; 
%daqOutFile=[daqNameCommon int2str0vec(daqdatindex)];
...
notifyWhenExceedsSamps=srate/10;
set(NIsession,'NotifyWhenDataAvailableExceeds',notifyWhenExceedsSamps);
 %2560
devs=daq.getDevices;
...
[lh]=NIsession.addlistener('DataAvailable', @dataaq);
...
%nsecs=2600; % das geht noch (3/4 Stunde)
nsecs=240;
datsize=nsecs*srate;

disp('Socket initialization ...')
[s] = wave_connect(2900)
%s.getSoTimeout
...
disp(char(p.data'));
% vieilleicht bessere Latenz??
s.setTcpNoDelay(true)

qStart=wave_assembleStartRecordingPacket;
...
while ~all_done
    ...
    while ~finished
        ...
        if mycmd=='n'
            ...
            %pack
            nSampsAcquired=0;
            disp('Starting NIDAQ aquisition');
            daqdatindex=daqdatindex+1; 
            daqOutFile=[daqNameCommon int2str0vec(daqdatindex)];
...
%                 soundsc(wavend_d,wavend_sf,wavend_bits);
            ...
            % Stopping NDI WAVE
            pStopMess = wave_negPackage (s,qStop);
            disp('pStopMess')
            disp(pStopMess)
            pause(0.5)
...
maxex=max(eee(3:4));
if maxex>1
    disp(['Reducing font to fit [', num2str(1/maxex), ']']);
    %disp(1/maxex);

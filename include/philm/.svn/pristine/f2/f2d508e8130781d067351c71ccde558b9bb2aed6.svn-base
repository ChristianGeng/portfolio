...
% TIF2MAT_TRIAL_F Load video tif images, deinterlace, store as MAT files
% function tif2mat_trial_f(tifin,basename,synchfile,myset)
% tif2mat_trial_f: Version 13.12.2012
%
%   Description
...
functionname='TIF2MAT_TRIAL_F. Version 13.12.2012';
        ...
        if ~isempty(tifin)
            ...
            for ii=1:nframe
                ...
                if exist(myname,'file')
                    ...
                    x=imread(myname);
                    %image adjustments:
                    %note: would be more efficient if cropping done earlier
            ...
            if graphok
                ...
                for ii=1:nframe set(hi,'cdata',data(:,:,:,ii)); drawnow; end;
            end;
            ...
            datax=deinterlace(squeeze(data(:,:,1,:)),ifirst,flipflag);
            %deinterlace changes the size
            dsize=size(datax);
            nrow=dsize(1);ncol=dsize(2);
            nframe=dsize(end);
            
            data2=repmat(uint8(0),[nrow ncol nplaneout nframe]);
            data2(:,:,1,:)=datax;
            if truecolflag
                data2(:,:,2,:)=deinterlace(squeeze(data(:,:,2,:)),ifirst,flipflag);
                data2(:,:,3,:)=deinterlace(squeeze(data(:,:,3,:)),ifirst,flipflag);
            end;
            ...
            if resizefactor~=1
                disp('Resizing');
                data2x=data2;
                %do a dummy frame to determine resulting size
                datatmp=data2x(:,:,1,1);
                datatmp=imresize(datatmp,resizefactor);
                nrow=size(datatmp,1);
                ncol=size(datatmp,2);
                ...
                for iplane=1:nplaneout
                    for ii=1:nframe
                        datatmp=data2x(:,:,iplane,ii);
                        data2(:,:,iplane,ii)=imresize(datatmp,resizefactor);
                    end;
                end;
            ...
            if ~isempty(wienersize)
                ...
                for iplane=1:nplaneout
                    for ii=1:nframe
                        datatmp=data2(:,:,iplane,ii);
                        data2(:,:,iplane,ii)=wiener2(datatmp,wienersize);
                    end;
                end;
            end;
            ...
            if ~isempty(timecoff)
                disp('Time filtering');
                for iplane=1:nplaneout
                    data2(:,:,iplane,:)=timefilt(squeeze(data2(:,:,iplane,:)),timecoff);
                end;
            ...
            data=squeeze(data2);        %remove singleton 3rd dimension for grayscale
            descriptor='VIDEO';

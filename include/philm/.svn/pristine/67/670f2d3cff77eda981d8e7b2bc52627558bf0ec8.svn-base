...
functionname='comppos_fm: Version 03.04.2013';
...
%needs changing!!!
%only seems to actually be needed when input from altpath as this is
%regarded as "additional" channels
%changed below, by using number of channels in main input as offset
    ...
    if ~isempty(pbig1)
        ...
        if ~isempty(altpath)
            if mytrial~=lasttrial
                ...
                sensorlist=str2mat(sensorlist,sensorlist2(kanallistb,:));

function daqscope_ph(varargin)
% DAQSCOPE_PH Example oscilloscope for the Data Acquisition Toolbox (Phil's version of daqscope)
% function daqscope_ph(varargin)
% daqscope_ph: Version 7.5.07
%
%   Description
%       Based on DAQSCOPE
%       Currently set up mainly for use in EGG demo.
%       Some simple customization is possible using the function myset
%       included at the end of the source code.
%       This can be called from keyboard mode; the program enters keyboard
%       mode whenever acquistion is halted.
%       Examples:
%       myset('title',data,'mytitle'): This inserts a title. Useful when
%       the program is called several times, and several different waveform
%       examples are on the screen
%       myset('line',data,'linewidth',2): Any line properties of the
%       oscillogram can be set in this way
%       When the program starts it is also possible to stretch the size of
%       the figure (see start of subfunction localinitfigure), e.g
%       enter stretch=1.5 in keyboard mode
%
%   Original DAQSCOPE help text
...
%    $Revision: 1.9 $  $Date: 2002/04/04 22:11:51 $
...
set(data.ai,...
   ...
   'Stopfcn', {'daqscope_ph', 'stopaction', gcbf});
...
range=sampleRate/10;
xdiv=range/10;
xdivms=round((xdiv/sampleRate)*1000);
xdivms=max([xdivms 1]);
rangems=xdivms*10;
range=(rangems/1000)*sampleRate;
   ...
   set(data.handle.axes,...
      ...
      'XTickLabel'        ,{'0','','','','','','','',num2str(rangems)}); 
   
   % Update x-axis slider to range from 0 to sampleRate/10. in ms (Phil)
...
xdivms=round(val/10);
   xdivms=max([xdivms 1]);
rangems=xdivms*10;
%set(data.handle.uicontrol(4), 'Value',rangems);
range=(rangems/1000)*sampleRate;
...
%simplest way at the moment of adjusting figure size
%could also include a setting here for things like y polarity
...
XOffset=25;
figWidth=580*xstretch+XOffset;

YOffset=0;
figHeight=315*ystretch+20;
...
hFig=figure(genInfo,...                    
   'Color'             ,btnColor                    ,...
   'DeleteFcn'         ,'daqscope_ph(''close'', gcbf)',...
   'DoubleBuffer'      ,'on'                        ,...
   'IntegerHandle'     ,'off'                       ,...
   'MenuBar'           ,'none'                      ,...
   'Name'              ,'Osci'   ,...
...
% Create the axes.
axPos = [46+XOffset 26+YOffset 367*xstretch 272*ystretch];
...
% Create the frames.
framePos = {[420*xstretch+XOffset 26+YOffset 158 272], [425*xstretch+XOffset 75 148 60]};  
...
popPos = [430*xstretch+XOffset 266+YOffset 138 22];  
...
h(1) = uicontrol(uiInfo,...
   ...
   'String'           ,adaptorStr   ,...
   'BackGroundColor'  ,[1 1 1]      ,...
   'Callback'         ,'daqscope_ph(''adaptor'', gcbf);');

% Create the channel listbox.
listPos = [430*xstretch+XOffset 169+YOffset 138 87];    
...
h(2) = uicontrol(uiInfo,...
   ...
   'String'          ,names            ,...
   'BackGroundColor' ,[1 1 1]          ,...
   'Callback'        ,'daqscope_ph(''changeChannel'', gcbf);');

% Create the edit text box - Volts/Div.
EditPos = [508*xstretch+XOffset 110+YOffset 60 20];
...
h(3) = uicontrol(uiInfo,...
   ...
   'Callback'             ,'daqscope_ph(''voltsrange'', gcbf);');

% Label the radiobutton - Volts/Div.
textPos = [430*xstretch+XOffset 110+YOffset 78 20];  
...
h(8) = uicontrol(uiInfo,...
   ...
   'Units'                ,'pixels'      ,...
   'Position'             ,textPos       ,...
   'Callback'             ,'daqscope_ph(''voltscheck'', gcbf);');

% Label the slider.
textPos = [430*xstretch+XOffset 50+YOffset 124 20];
...
% Create the slider.
sliderPos = [430*xstretch+XOffset 35+YOffset 138 20];
...
h(4) = uicontrol(uiInfo,...
   ...
   'Units'                ,'pixels'              ,...
   'Position'             ,sliderPos             ,...
   'Callback'             ,'daqscope_ph(''axisrange'', gcbf);');

% Create the SampleRate checkbox.
CheckBoxPos=[430*xstretch+XOffset 140+YOffset 78 20];
...
h(5) = uicontrol(uiInfo,...
   ...
   'String'               ,'SampleRate'          ,...
   'HorizontalAlignment'  ,'left'                ,...
   'Callback'             ,'daqscope_ph(''checkbox'', gcbf);');

% Create the SampleRate edit text box.
sampleRate = get(ai, 'SampleRate');
EditPos=[508*xstretch+XOffset 140+YOffset 60 20];
...
h(6) = uicontrol(uiInfo,...
   ...
   'HorizontalAlignment'  ,'left'                ,...
   'Callback'             ,'daqscope_ph(''sampleRate'', gcbf);');

% Create the autoset radiobutton.
RadioPos=[430*xstretch+XOffset 80+YOffset 124 20];
...
h(7) = uicontrol(uiInfo,...
   ...
   'Position'             ,RadioPos              ,...
   'String'               ,'Autoset'             ,...
   'Callback'             ,'daqscope_ph(''autoset'', gcbf);');
...
h(9) = uicontrol(uiInfo,...
   ...
   'Callback'             ,'daqscope_ph(''startpause'',gcbf);');
%Create the stop bar 
h(10) = uicontrol(uiInfo,...
   ...
   'Callback'             ,'daqscope_ph(''stop'', gcbf);');
...
menu(2) = uimenu(menu(1),...
   ...
   'Callback'           ,'daqscope_ph(''close'', gcbf);');
...
menu(4) = uimenu(menu(3),...
   'Label'              ,'Legend'                  ,...
   'Callback'           ,'daqscope_ph(''legend'',gcbf);');
...
menu(6) = uimenu(menu(5),...
   ...
   'Callback'           ,'daqscope_ph(''oshelp'', gcbf);');
menu(7) = uimenu(menu(5),...
   'Label'              ,'Data Acquisition Help' ,...
   'Callback'           ,'daqscope_ph(''daqhelp'', gcbf);');
...
htmp=get(hAxes,'title');
set(htmp,'fontsize',16,'fontweight','bold');
htmp=get(hAxes,'xlabel');
set(htmp,'string','t (millisec.)','fontsize',14,'fontweight','bold');
set(htmp,'vertical','bottom')
set(hAxes,'fontsize',14,'fontweight','bold');

%seems to be necessary for egg via soundcard
...
mycmd=varargin{1};
if strcmp(mycmd,'title')
data=varargin{2};
    htit=get(data.handle.axes,'title');
    set(htit,'string',varargin{3});
end;

if strcmp(mycmd,'line')
data=varargin{2};
    hlines=data.handle.line;
    set(hlines,varargin{3},varargin{4});
end;

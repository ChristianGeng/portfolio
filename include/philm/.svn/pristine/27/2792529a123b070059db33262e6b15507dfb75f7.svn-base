function [yo,fo,to] = specgram_lpc(varargin)
% SPECGRAM_LPC Calculate spectrogram from signal. Optional LPC and MTM spectrum
% function B = SPECGRAM_LPC(A,NFFT,Fs,WINDOW,NOVERLAP)
% SPECGRAM_LPC: Version 27.11.09
%
%	Description
%	calculates the spectrogram for
...
%
%   Phil's changes
%       Can be called with a 6th argument
%           If this is a scalar then this flags the use of adaptive
%           pre-emphasis
%           If two-element vector then first element controls pre-emphasis
%           and absolute value of 2nd element is LPC filter order. If positive then only the
%           LPC coefficients are returned (e.g for formant calculation in
%           mtsona2file). In this case the NFFT specification is
%           irrelevant.
%           If negative, the LPC spectra are returned
%			If string: currently only 'pmtm' possible. To allow alternative psd methods
%			Note: pmtm (and in future maybe other psd methods) returns the
%			real-valued power spectrum, whereas normal fft spectrograms
%			return the complex spectrum. See source code below for notes on
%			scaling of output
%
%   See also PWELCH, CSD, COHERE and TFE, MTSONA2FILE
%
...
%   $Revision: 1.5 $  $Date: 1999/01/11 19:05:11 $

%error(nargchk(1,5,nargin))
error(nargchk(1,6,nargin))			%phil
V=varargin;

%Phil: handle pre-emphasis/ lpc specification
%default for lpcord must be handled by calling routine
dolpc=0;
domtm=0;
preempflag=0;
if length(V)==6
...
[msg,x,nfft,Fs,window,noverlap]=specgramchk(V);
error(msg)
...
t = (colindex-1)'/Fs;
f=[];       % Unused if returning lpc coefficients
...
if (length(nfft)==1) | use_chirp
	...
	%adaptive pre-emphasis (framewise)
	if preempflag==1
		for ii=1:ncol
			ytmp=y(:,ii);
			myR=xcorr(ytmp,ytmp,1);
			empfac=myR(1)/myR(2);
			ytmp=filter([1 -empfac],1,ytmp);
			y(:,ii)=ytmp;
		end;
	end;
	...
	if ~use_chirp     % USE FFT
		...
		if dolpc
			lpccof=lpc(y,abs(lpcord));
			%each ROW of lpccof has the lpc coefficients for each chunk of data
			if lpcord>0
				y=lpccof';
				yo=y;
				fo=f;
		...
		if domtm
			disp('specgram_lpc: doing mtm');
			for icol=1:ncol
				[Pxx,f] = pmtm(y(:,icol),[],[],Fs);
				if icol==1
					yout=ones(length(f),ncol);
				end;
				yout(:,icol)=Pxx;
			end;
			y=yout;

%pmtm scales the whole result by Fs, and for the usual case of even length fft also scales the result as follows
%(see computepsd)
...
%this scaling of first and (usually) last point for normal fft is
%replicated rather clumsily below

%remove psd scaling by Fs for compatibility with normal FFT
%assume pmtm has also scaled by window length (though I haven't yet found
%where), so remove this too, as this is done again below
			y=y*nwind*Fs;
			%normal fft
		else
			y = fft(y,nfft);
			if ~any(any(imag(x)))    % x purely real
				if rem(nfft,2),    % nfft odd
					...
					select = [1:nfft/2+1];
					y(1)=y(1)/2;
					y(select(end))=y(select(end))/2;
				end
				y = y(select,:)*2;		%handle fft symmetry for real signals (exceptions handled by previous division above)
				if dolpc y=1./y; end;
			else
				select = 1:nfft;
			end
...
%not sure this is appropriate for all possible methods.....
y=y/nwind;

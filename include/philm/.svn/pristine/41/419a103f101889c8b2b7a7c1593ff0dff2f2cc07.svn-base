function do_kalman_rtmon(chanlist,igo,istep)
% DO_KALMAN_RTMON run tapad online during experiment
% function do_kalman_rtmon(chanlist,igo,istep)
% do_kalman_rtmon: Version 16.3.09
%
%   Description
...
%       Currently 'kalmanrt' is added to the output path.
...
%    outpath=[firstpath pathchar 'beststartl' pathchar 'rawpos'];
    outpath=[firstpath pathchar 'kalman' kalmansuffix pathchar 'rawposm'];
...
%            stats=tapad_ph_rs(basepath,[firstpath pathchar 'amps'],outpath,trial2do,chanlist,myoptions);
			stats=kalman_fb(firstpath,kalmansuffix,trial2do,chanlist);

		else
            %with startpos
%            stats=tapad_ph_rs(basepath,[firstpath pathchar 'amps'],outpath,trial2do,chanlist,myoptions,startpos);
			stats=kalman_fb(firstpath,kalmansuffix,trial2do,chanlist,startpos);

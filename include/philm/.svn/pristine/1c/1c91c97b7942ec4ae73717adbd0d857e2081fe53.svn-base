function prompter_parallel(stimfile,logfile,trial_duration,fontsize,startwav,endwav,mysynchline,transshiftstruc);
% PROMPTER_PARALLEL Prompt program with synch signal output on parallel port. Will also run when port n.a
% function prompter_parallel(stimfile,logfile,trial_duration,fontsize,startwav,endwav,mysynchline,transshiftstruc);
% prompter_parallel: Version 09.12.2011
%
%	Syntax
...
%           Set to 0 to disable
%       transshiftstruc: Structure with settings specific to prompter_parallel_transshift
%           Currently only one field is definded:
%           transshiftstruc.sessionname
%           contains filename (without trialnumber) to use for data output
%
%   Description
...
functionname='PROMPTER_PARALLEL_TRANSSHIFT: Version 09.12.2011';
...
outfilename='';
TSS=[];
if nargin>7 TSS=transshiftstruc; end;
if isfield(TSS,'sessionname') outfilename=TSS.sessionname; end;

transshift_ctrl_1('ini',outfilename);
...
hidemode=1;     %suppress additional on-screen message to subject, and immediate display of next prompt
delaystart=1;   %extra keypress inside 'next' command to start recording
%               e.g if hidemode ==1 prompt is not visible until 'next'
%               command
if ~synchline
    daqok=0;
else
...
helpwin prompter_parallel_transshift;
...
while ~all_done
    ...
    while ~finished
        ...
        if mycmd=='n'
            ...
            hidep(1); %make stimulus visible on subject screen if it has been hidden
            %set traffic lights to ready? or make prompt visible
            dotrafficlights(TL.getready,ht,figh);
            tmpstr=cs{istim};
            %assume first part of code string is the actual code, the rest is settings for transshift
            [tmp2,tmpstr]=strtok(tmpstr,'#');
            transshift_ctrl_1('set',irun,tmpstr);
            ...
            %allow for another pause until subject actually starts??
            if delaystart disp ('Hit any key to start '); pause; end;
            ...
            if isempty(trial_duration)
                ...
                while toc<trial_duration
                    ...
                    pause(0.050);       %no point in updating any faster
                    %e.g do also do time bar in subject window
...
function showg(ht,mys,hidemode);

%imageprefix='<IMAGE>';

%keep new stimulus invisible, if hidemode is true
turnoff=0;
if nargin>2 turnoff=hidemode; end;
...
%	'a = autonext',...
%	't = set trial number',...
function hidep(showflag)
%only hide in subject figure

myflag=0;
if nargin myflag=showflag; end;
hfsub=findobj('type','figure','name','Subject');
P=get(hfsub,'userdata');

ht=findobj(hfsub,'type','text','tag','prompt');
if myflag
    if strcmp(P.promptmode,'text') set(ht,'visible','on'); end;
else
    set(ht,'visible','off');
end;

function prompter_parallel_trigag(stimfile,logfile,trial_duration,fontsize,startwav,endwav);
% PROMPTER_PARALLEL_TRIGAG Stimulus display; trigger and monitor AG500; video timer control
% function prompter_parallel_trigag(stimfile,logfile,trial_duration,fontsize,startwav,endwav);
% prompter_parallel_trigag: Version 27.08.2011
%
%	Syntax
...
%               'wavlist' in the source code below (12.09: code introduced with flag for choice of
...
%		LIDA_CONNECT, GETEMAALL, TOGGLESTATUS for control of AG500 via
%		tcpip
%		LIDA_RTMON Real-time monitoring of AG500 positions
%
%   Updates
%       Delay in auto mode
%       8.08 First version of prompter_parallel_trigag based on
%       prompter_parallel_trigin. Controls start and stop of Linux AG500
%       recording and monitors current trial number on Linux system. Also displays some simple
%		information on raw ag500 signal levels
%		8.10 add time-out when waiting for sweep after attention
%       08.11 Add userstart/stop functions (see source code for details)
%
%   Details
%       Monitors ag500 status signals from sybox
%       Remotely starts cs5recorder on AG500 control server
%			Trial duration is controlled by input argument trial_duration.
%			So the 'Carstens' linux command setsweeplen should not be used
%			on the control server (i.e should be left at its default
%			maximum value. One slight disadvantage: The way
%			prompter_parallel_triginag controls trial duration means that
%			the actual trial duration will not be as precise as if
%			setsweeplen was used.
%			In fact, a trial can also be terminated by hand before the pre-set trial duration using
%			cs5recorder (because prompter_parallel_trigan, as in older
%			versions, still monitors and reacts to the sweep signal from
%			the sybox). However, it is not yet really clear whether this
%			can confuse prompter_parallel_trigag.
%       Incorporates video timer control and synch output from PC
%       Prompt output also to possible to serial monitor (see COMPROMPT)
...
functionname='PROMPTER_PARALLEL_TRIGAG: Version 27.08.2011';
...
vtgtrialfix=1;      %If true, trial number always 1 (also to speed up, if trial number not essential)
...
sweep_timeout=5;	%not clear why this is necessary, assuming attention has been correctly detected
...
%use to terminate trial before end of normal duration
set(hfs,'userdata',0,'windowbuttondownfcn','promptcb');
...
[active,sample,sweep,dataS,dataC,pos] = getEmaAllf;



disp('AG500 Positions (x, y, z, phi, theta, rms, spare');
disp(pos);
disp('AG500 Amplitudes*1000');
disp(round(dataS*1000));
disp(['Active, samplenum, trialnum : ' int2str([active sample sweep])]);
...
helpwin prompter_parallel_trigag;
...
while ~all_done
	...
	while ~finished
		...
		if mycmd=='n'
			...
			if ~ok2rec
				...
				if triggered
					...
					%output to log file
					status=fwrite(fid,['!!Sweep already running; check trialnumbers are consecutive !!' crlf],'uchar');
					...
					if chstat(A.sweep)
						...
						status=fwrite(fid,['!!agmaster sweep without prompt !!check trialnumbers are consecutive' crlf],'uchar');
				...
				if chstat(A.trans) & ~agstat(A.sweep)
					...
					disp('Check transmitter status before continuing');
				...
				disp('x y z phi theta rms(resid) spare rms(amp)*1000');
				disp([pos pegel]);
				disp(['Active, samplenum, trialnum : ' int2str([active sample sweep])]);
				...
				if ~isempty(irunold)
					...
					if irunold<(irun-1)
						...
						status=fwrite(fid,['!!Dummy trials inserted!!' crlf],'uchar');
						...
						disp('Dummy trials inserted in log file');
						disp('Type return to continue');
						keyboard;
					end;
				end;
            ...
            if ~isempty(userattnfunc)
                eval(userattnfunc,'disp(''Error evaluating userattnfunc'');disp(lasterr)');
					...
					while ~triggered & ~timeoutsweep
						...
						triggered=~agstat(A.sweep);
						tcheck=toc;
						timeoutsweep=tcheck>sweep_timeout;
					end;
					if timeoutsweep
						dotrafficlights(TL.stop,ht,figh);
						disp('Timed out waiting for sweep');
						disp('Type return to continue');
						%                    disp('Use "t" command to set trial number to number of next AG trial');
						%output to log file
						status=fwrite(fid,['!!Timed out waiting for sweep !!' crlf],'uchar');
						keyboard;
						resetagstatus(hlstat);
					else
            ...
            set(htinfo(1),'string',[iruns ' : ' num2str(toc,'%4.3f')],'color',htinfostopcol);
						...
						irunold=irun;
						set(hfs,'userdata',0);		%reset click to stop flag in status figure
		...
		if ((mycmd=='r') | (mycmd=='R'))
			...
			%R no longer makes sense when getting irun from tcpip
			%			if mycmd=='R'
			%				irun=irun-1;
...
delete([hfsub;hfinv]);
delete(hfs);
%not needed as always closed immediately after use
%s.close;            %tcpip connection
...
function [active,sample,sweep,dataS,dataC,pos] = getEmaAllf;
lidatimeout=200;
readok=0;
retry = 0;
while ~readok
	try
		...
		if (retry >9)
			disp(lasterr);
			disp(['No connection??' crlf 'Type return when ready to reconnect!']);
			disp('First make sure cs5recorder is running');
			keyboard;
			retry=0;

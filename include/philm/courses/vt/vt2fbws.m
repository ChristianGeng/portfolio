...
%
%   Updates
%       Version for continuous sound output using data acquisition toolbox
%       (Windows only). If daq toolbox not available uses soundsc.
%       If used on Linux matlab v7 or later is needed for soundsc to work
%       with the appropriate samplerate (use vt2fbws_old if problems occur)
%
%   See Also
...
%use data acquisition toolbox for continuous sound output, if possible
if strcmp('PCWIN',computer)

    sv=ver;
    svn=cell(length(sv),1);
    [svn{:}]=deal(sv.Name);
    vv=strmatch('Data Acquisition',svn);
    if length(vv)==1
        usedaq=1;
    end;
end;
...
slen=0.5;               %length of sound chunk
f0=100;

%taper when not using continuous output
ntaper=200;
hann1=hann(ntaper*2);
hann1=hann1(1:ntaper);


%f0end=f0-(f0*.2);
f0end=f0;

%generate glottal pulse at higher samplerate and then resample to avoid
%sharp discontinuities
tmpfac=8;
x=glottit(f0,f0end,slen,samplerate*tmpfac);
myglott=x';
myglott=resample(myglott,1,tmpfac);
clear x;
myglotts=myglott(2:end);
%Use negative zero crossings to delimit section to use after filtering with
%the transfer function (helps to avoid artefacts in continouous audio
%output)
vv=find(myglotts<0 & myglott(1:(end-1))>=0);
mypick=[vv(2) vv(end-1)];


fuse=5;     %formants to use
...
if usedaq
    try
        ...
        disp('Tried to use daq for sound output, but may not be available');
        usedaq=0;
    end;
end;
if usedaq
    %This makes one channel available for use
    hchanout=addchannel(hao,1);
    set(hao,'standardsamplerates','off');
    set(hao,'samplerate',samplerate);
    samplerate_in=samplerate;
    samplerate=get(hao,'samplerate');


    %samplecount for callback
    nsampcount=mypick(2)-mypick(1)+1;

    %Queues data for output by the DA-Converter
    myzeros=zeros(nsampcount,1);
    SOUNDDATA=myzeros;
    putdata(hao,[SOUNDDATA;SOUNDDATA]);
    set(hao,'samplesoutputfcn',@soundoutcb);
    set(hao,'samplesoutputfcncount',nsampcount);
end;
...
xlabel('Distance from glottis (cm)');ylabel('Area (cm^2)');
grid on;
title('Area Function');
...
title('Spectrum: red=tractsondhi, green=impfreqz match');
...
collist='rgbcmy';
%Note: Plotted as 3D. But when is this useful?
for ifi=1:fuse
...
xlabel('Constriction position (cm)');zlabel('Constriction size (cm^2)');
title('Nomogram');
...
while ~alldone
    ...
    if any(areadata~=lastarea)
        ...
        mysoundx=filter(b,a,myglott);
        mysoundx=mysoundx(mypick(1):mypick(2));
        mysoundx=(mysoundx/(max(abs(mysoundx))))*0.9;
        SOUNDDATA=mysoundx;

        if usedaq
            haocheck=get(hao,'sending');
            if strcmp(haocheck,'Off')
                disp('Restarting audio');
                putdata(hao,[SOUNDDATA;SOUNDDATA]);
                start(hao);
            end;
        end;
    ...
    if donomo
        ...
        %note: constriction location of nomogram is really determined by position of last mouse
        %click, not really by determining midpoint of constriction
        lastp=myS.lastpoint;
        for ifi=1:fuse
    ...
    if usedaq
        ...
        soundtmp=mysoundx;
        soundtmp(1:ntaper)=soundtmp(1:ntaper).*hann1;
        soundtmp=flipud(soundtmp);
        soundtmp(1:ntaper)=soundtmp(1:ntaper).*hann1;
        soundtmp=flipud(soundtmp);

        sound(soundtmp,samplerate);
        %under windows soundsc returns immediately, but not under linux
        mypause=slen+0.1;
        if isunix mypause=0.1; end;
        pause(mypause);
    end;
...
if fileused
    ...
    %      disp(wavfile);
    wavwrite(mysoundx,samplerate,16,wavfile);
end;
...
%6.2011. changed determination of section in which the mouse is located
tmplen=[x(1);diff(x)];
xx=x-tmplen/2;
%[minw,minp]=min(abs(x-cp(1)));
%curx=x(minp);
[minw,minp]=min(abs(xx-cp(1)));
curx=xx(minp);
...
function soundoutcb(cbobj,cbdata);
global SOUNDDATA
%sa=get(cbobj,'samplesavailable');
putdata(cbobj,SOUNDDATA);
%if sa<(length(SOUNDDATA)/4)
%putdata(cbobj,SOUNDDATA);
%end;
function x=glottit(f0,f0end,slen,samplerate);

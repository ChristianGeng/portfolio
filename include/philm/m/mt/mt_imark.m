...
%			available.
%           Note that if the marker file already contains markers whose type is greater
%           than nmark, then they will not be loaded or displayed
%
%		markfile
...
%Note: 12.09 mt_imark should probably call mt_rmark on completion to make
%sure any existing markers for current cut are read in (otherwise multiple
%segmentations are possible for the cut active when mt_imark is called, even if in 'E' mode)
...
functionname='mt_imark: Version 21.9.09';
...
if nargin>5 helpwin([crlf create_comment crlf],'MT_IMARK: Creation comment to marker file');end;
...
%must eliminate any cut types <=0
	v0=find(cut_type_value<=0);
	if ~isempty(v0);
	cut_type_value(v0)=[];
	cut_type_label(v0,:)=[];
	end;
    ...
    vvx=find(cut_type_value<=nmark);
    cut_type_valuex=cut_type_value(vvx);
    tmplabel=tmplabel(vvx);
	tmpcell(cut_type_valuex)=tmplabel;
%keyboard;

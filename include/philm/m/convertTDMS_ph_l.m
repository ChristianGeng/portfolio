function [ConvertedData,ConvertVer,ChanNames,GroupNames,ci]=convertTDMS_ph(filelist,chanlist,outformat,scalefactor)
% function [ConvertedData,ConvertVer,ChanNames,GroupNames,ci]=convertTDMS_ph(filelist,chanlist,outformat,scalefactor)
% CONVERTTDMS_PH Function to read LabView TDMS data file(s). Store selected channels as simple binary files
% CONVERTTDMS_PH Version 06.12.2012
%
%   Description
%       Based by Phil Hoole on function retrieve from matlab file exchange
%       (original help text below).
%       Main changes:
%       Intended to extract single channels for storage as simple BIN file.
%       These long BIN files can then be divided into single trial-based
%       mat files by daq2mat.
%       A mat info file is created with the same name as the BIN file with
%       '_info' as suffix. This contains information on the format of the
%       bin file, and additional information on the channel from the TDMS
%       file.
%       Only data with a decimation level of 0 are exported in this way.
%       Data with decimation level > 1 are skipped completely.
%       Data with no decimation level information (e.g. events?) are returned as in the
%       original matlab function
%
%   Syntax
%       Chanlist: list of channels to export. They are identified by their
%       name as found e.g. in the tmdms _meta.txt file (the last part of
%       the logged signal name after the final '-'). Valid names can also
%       be found in the ChanNames output argument of this function.
%
%   Updates
%       12.2012 First attempt to divide very long segments into chunks when
%       reading and writing (otherwise matlab may run out of memory)
%       Not sure when this occurs: possibly when tdms file only has a
%       single channel, so signalexpress may just need to update the amount
%       of data in each segment when writing a chunk, rather than writing
%       new segment information each time.
%       Also write a mat file called seginfo containing the seginfo struct
%       from the first scan of the file (just designed for help in
%       debugging if any read errors occur with the actual data)
%
%   See Also DAQ2MAT, GETDAQSYNC, FINDTDMS_CHINFO
%
%   Original Help
%       A *.MAT file can also be created.  If called with one input, the user selects
%       a data file.
%
...
functionname=['converttdms_ph_l: Version 06.12.2012 (from original version ' ConvertVer ')'];
ConvertedData=[];
SaveConvertedFile=1;
...
for fnum=1:nfile
    FileNameLong=deblank(filelist(fnum,:));
    
    if ~exist(FileNameLong,'file')
        e=errordlg(sprintf('File ''%s'' not found.',FileNameLong),'File Not Found');
        uiwait(e)
        return
    end
    ...
    ob=getData(fid,channelinfo,chanlist,outformat,scalefactor,functionname);
    fclose(fid);
...
while (ftell(fid) ~= eoff)
    ...
    if Ttag==84 && Dtag==68 && Stag==83 && mtag==109
        ...
        %        disp(round([segCnt CurrPosn TotalLength]));
    end
...
NumOfSeg=segCnt;
%Store segment information. May help debugging if any problems reading
%actual data
save('tdms_seginfo','SegInfo');
end
...
for segCnt=1:NumOfSeg
    ...
    if (kTocRawData) %only do the check if there was raw data in the segment
        ...
        if (offset~=SegInfo.DataLength(segCnt))
            ...
            disp(['Setting multiplier field : ' int2str(multiplier)]);
            for kk=1:numel(objOrderList)
                obname=objOrderList{kk};
                ccnt=index.(obname).rawdatacount;
...
function ob=getData(fid,index,chanlist,myouttype,myoutfac,functionname)
ob=[];
fnm=fieldnames(index);
for kk=1:length(fnm)
    id=index.(fnm{kk});
    ...
    if id.rawdatacount>0
        cname=id.name;
        ob.(cname).nsamples=0;
        [filename,declevel,INFO]=findtdms_chinfo(id);
        disp([int2str(kk) ' ' filename '. dec level: ' int2str(declevel)]);
        mydescrip=INFO.descriptor;
        
        %    keyboard;
        %notes on output factor:
        %for export of signals to mat files than scaling will always be 1
        %Unfortunately, although audacity can read floating point single data it
        %seems to want them in the range +/-1, so to generate an audio file with
        %subject, investigator and maybe sync channels its probably best to do an
        %int16 version scaled appropriately.
        %Default scaling for integer output will need to look at AI.max/min fields
        %in the project xml file (or maybe by storing the name of the ai input
        %module somewhere??)
        %Digital signals contain 0/1 (as an uint8 value), so scaling is
        %really irrelevant for use in matlab (but for reading in audacity
        %like an audio signal scale e.g by 100 and store and int8)
        ...
        INFO.scalefactor=myoutfac;
        INFO.bindatatype=myouttype;
        %store info as mat file
        filenamea=[filename '_' myouttype];
        doexport=0;
        if strmatch(mydescrip,chanlist) doexport=1; end;
        if declevel==0 & doexport
            fido=fopen([filenamea  '.bin'],'w');
            %store complete structure; could maybe try and convert to more readable
            %form first
            %There are various hardware settings that only seem to be documented if a
            %project xml file is generated.
            %Might be worth including it here. Also parse for
            %essential information and store in every resulting mat file?
            
            INFO.tdmsinfo=id;
            %functionname=???
            comment='Info on corresponding .BIN file';
            comment=framecomment(comment,functionname);
            
            save([filenamea '_info'],'INFO','comment');
        end;
        ...
        for rr=1:id.rawdatacount
            ...
            nvals=id.nValues(rr);
            if nvals>0
            disp([rr id.rawdatacount nvals id.multiplier(rr)]);
%phil: the original version read nvals*multiplier values in one read.
%However, in some cases (currently suspected to be simple single-channel
%recordings) this caused out of memory errors. i.e. signal express
%sometimes writes very large segments.
%See also the notes above where the multiplier is set up in getchannelinfo
%(The original programmers apparently suspected that the nValues parameter
%was not always set correctly)
%Currently, this version of the program cannot handle string or timestamp data if multiplier is ~=1.
%But I'm not sure whether this happens in practice. And even if it does the
%program version without the multipler loop (convertTDMS_ph) cannot almost
%certainly handle it, as the segments are unlikely to ever be as enormous
%as they can be for waveform data

            for ichunk=1:id.multiplier(rr)
                    
                    switch id.dataType
                        
                        case 32		%String
                            if id.multiplier(rr)==1
                            ...
                            for mm=1:id.multiplier(rr)
                                StrOffsetArray=fread(fid,nvaluse,'uint32');
                                for dcnt=1:nvaluse
                                    if dcnt==1
                                    ...
                                    data{1,dcnt+(mm-1)*nvaluse}=char(convertToText(fread(fid,StrLength,'uint8=>char'))');
                                end
                            ...
                            cnt=nvals*id.multiplier(rr);
                                ...
                                disp('Use convertTDMS_ph instead for this data');
                                keyboard;
                            end;
                        ...
                        case 68		%Timestamp
                            if id.multiplier(rr)==1
                            %data=NaN(1,nvaluse);	%Pre-allocation
                            data=NaN(1,nvals*id.multiplier(rr));
                            for mm=1:id.multiplier(rr)
                                dn=fread(fid,2*nvaluse,'uint64');
                                tsec=dn(1:2:end)/2^64+dn(2:2:end);
                                data((mm-1)*nvaluse+1:(mm)*nvaluse)=tsec/86400+695422-4/24;
                                fseek(fid,id.skip(rr),'cof');
                            end
                            %{
						for dcnt=1:nvaluse
                        ...
                        otherwise	%Numeric
                            ...
                            if (id.skip(rr)>0)
                                ntype=sprintf('%d*%s',nvaluse,matType);
                                if strcmp(matType,'uint8=>char')

%phil: changes here for loop through multiplier
%                                    [data,cnt]=fread(fid,nvaluse*id.multiplier(rr),ntype,id.skip(rr));
                                    [data,cnt]=fread(fid,nvals,ntype,id.skip(rr));
...
%                                [data,cnt]=fread(fid,nvaluse*id.multiplier(rr),matType);
                                [data,cnt]=fread(fid,nvals,matType);
                            end
                    end                 %switch id.dataType
